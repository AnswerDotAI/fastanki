...
def sync(self:Collection, user=None, passw=None, media=True):
    ...
    self.save_auth(auth)
    if result.required in (result.ChangesRequired.FULL_DOWNLOAD, result.ChangesRequired.FULL_UPLOAD):
        self.close_for_full_sync()
        upload = result.required == result.ChangesRequired.FULL_UPLOAD
        self.full_upload_or_download(auth=auth, server_usn=result.server_media_usn, upload=upload)
...
class Deck:
    ...
    def add(self, model='Basic', tags=None, **fields): return self.col.add(model=model, deck=self.name, tags=tags, **fields)
